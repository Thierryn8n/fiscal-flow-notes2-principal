...
const NotesManagement: React.FC = () => {
  ...
  // Função para marcar uma nota como impressa ou enviar para fila de impressão
  const handlePrintAction = async (note: FiscalNote) => {
    ...
    if (isMobile) {
      ...
      try {
        ...
        toast({
          ...
          description: error.message || 'Não foi possível enviar o orçamento para a fila de impressão.',
          variant: 'destructive',
        });
      } finally {
        setIsLoading(false);
      ...
      // Em desktop, marcar como impressa diretamente
      handleMarkAsPrinted(note.id);
    }
  };
                                ...
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Button
                                      variant="outline"
                                      size="icon"
                                      onClick={() => handlePrintAction(note)}
                                      className="h-8 w-8 rounded-full"
                                      disabled={
                                        isMobile
                                          ? note.status === 'canceled'
                                          : note.status === 'printed' || note.status === 'finalized' || note.status === 'canceled'
                                      }
                                    >
                                  ...
                                  <TooltipContent>
                                    {isMobile ? 'Enviar para Impressão' : 'Marcar como impressa'}
                                  </TooltipContent>
                                </Tooltip>
